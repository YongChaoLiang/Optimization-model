...
def cnn(x_train, x_test, y_train, y_test, batch_size, epochs, filters, kernel_size, stride=1):
    try:
        ...
        test_pearson = stats.pearsonr(y_test.reshape(-1), test_pre.reshape(-1))  # 相关系数
...
def optimizeCNN(x_train, x_test, y_train, y_test, batch_size, kernel_size, particleDimensions, stride=1):
    try:
        numberFilters = int(particleDimensions[0])
...
if __name__ == "__main__":
    # Define the pso initialization parameters
    GLOBAL_BEST = 'G'
    LOCAL_BEST = 'L'
    C1 = 'c1'
    C2 = 'c2'
    ...
    # Call PSO particle swarm optimization CNN function
